...
<?php $YA1 = mysql_query("select * from jokedr_yoneticigirisi where No in ('1')"); $YA = mysql_fetch_assoc($YA1);?>
					<div class="banner">
						<h2><? echo "$menul_016";?></h2>
						<p><? echo "$yonet_002";?></p>
					</div>
					<h3><? echo "$yonet_002";?>:</h3>
					<div class="well">
						<strong><? echo "$yonet_003";?>:</strong>
						<br />
						<br />
						<form name="Guncelle" id="durum" method="post" class="class="form"" enctype="multipart/form-data" action="dahil/modul/digeri/yoneticiayarlari_ok.php">
							<label for="name"><? echo "$yonet_004";?></label>
							<input name="Eposta" class="span4" type="text" id="Eposta" value="<?=$YA['Eposta'];?>" />
							
							<label for="inputEmail"><? echo "$yonet_005";?></label>
							<input name="Sifre" class="span4" type="password" id="Sifre" value="<?=$YA['Sifre'];?>" />
							<br />
							<button type="submit" class="btn btn-success"><? echo "$yonet_003";?></button>
						</form>
					</div>
				</div>
			</div>
