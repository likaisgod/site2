...
<?php $TW1 = mysql_query("select * from zcard_twitter where No in ('1')"); $TW = mysql_fetch_assoc($TW1);?>
					<div class="banner">
						<h2><? echo "$menul_003";?></h2>
						<p><? echo "$twitt_001";?></p>
					</div>
					<h3><? echo "$menul_003";?>:</h3>
					<div class="well">
						<strong><? echo "$twitt_001";?>:</strong>
						<br />
						<br />
						<div class="alert alert-info"><h4><? echo "$panel_uyr";?>!</h4><p><? echo "$twitt_002";?></p></div>
						<form name="Guncelle" id="durum" method="post" class="class="form"" enctype="multipart/form-data" action="dahil/modul/digeri/twitter_ok.php">
							<label for="name"><? echo "$menul_003";?></label>
							<input name="TWApi" class="span4" type="text" id="TWApi" value="<?=$TW['Api'];?>" />
							<br />
